    ...
    export default {
		data : function(){
			return {
				...
				message : "",
				//用户头像
				userThumb : "../../assets/img/fans1.png",
				//用户id
				userId : "",
				//对方id
				otherId : "",
				//对方头像
				otherThumb : "../../assets/img/fans1.png",
				//对方名字
				otherName : "机器人",
				// loading
				loaded: false,
		...
		methods : {
				...
				ajax.getLog(id, that.page, function(res){
					if(res.status == 200){
						...
						for(var i=0;i<data.length;i++){
							...
							//如果为图片，则替换路径
							if(data[i].type == 1){
								data[i].msg.thumb = window.config.img_url + data[i].msg.thumb;
							}
			...
			openChat : function(){
				...
				//设置websocket连接参数
				websocket.setUrlParams("toid=" + this.$route.params.id + "&send_welcome=" + this.$route.params.send_welcome + "&handler=chatonline");
				...
				websocket.setOnMessage(function(data){
					...
					if(data.type == 1 || data.type == 2){
						data.msg.thumb = window.config.img_url + data.msg.thumb;
					}
				...
				plupload.init({
					...
					fileUploadBeginEvent : function(){
						...
						imageIndex = that.news.push({
							objclass: "chat-own",
							html: {
								thumb: "../../assets/img/pleasemakeitagain.png",
							},
							type: 1
						}); 
					...
					fileUploadEvent : function(up, file, info){
						if(info.status == 200){
							...
							that.news.splice(imageIndex, 1, {
								objclass : "chat-own",
								html : {
									thumb : window.config.img_url + file,
								},
								type : 1
							});
						...
						if (res.status == 200) {
							...
							that.news.push({
								...
								html: {
									...
									thumb: {
										url : window.config.img_url + res.data.thumb.url
									},
									id: res.data.id
								},
								type: 3
