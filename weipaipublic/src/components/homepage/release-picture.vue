	...
	<div class="picture-content">
		<div class="lot-picture">
			...
			<ul class="picture-list clearfix">
				...
				<!-- 微信浏览器 -->
				<li class="fl re divbg img-box" :data-index="index" v-for="(item,index) in imgs" v-if="isWeixin">
				...
				<!-- web浏览器 -->
				<li class="fl re divbg img-box" :data-index="index1" v-if="!isWeixin" v-for="(item1,index1) in imgList">
					<img class="preview-img list-img-ele fl" v-if="!item1.type" :src="item1.src+'?x-oss-process=image/resize,w_800'" height="100" @click="$preview.open(index1, imgList, option)" data-statistics="click_img">
					<div class="video-box" v-if="item1.type == 'video' " @click="showBigImg">
						<span class="video-play"></span>
						<video 
							class="my-video" 
							:src="baseUrl2+item1.src" 
							:data-src="item1.src" 
							data-type="video" 
							:poster="videoPoster"
						></video>
	...
	<div class="fill-form" v-if="true">
		<div class="fill-form-con" @click="bytime">
			<div class="clearfix fill-border">
				<label class="fl label">开拍时间</label>
				<div class="fl fill-money" id="bytime" >{{ fillData.startLine }}</div>
				<label class="fr fill-icon shotsprites"></label>
			</div>
		</div>
		<div class="fill-form-con" @click="deadlineEvent">
			<div class="clearfix fill-border">
				<label class="fl label">截止时间</label>
				<div class="fl fill-money" id="shooting-time">{{ fillData.deadLine }}</div>
				<label class="fr fill-icon shotsprites"></label>
			</div>
		</div>
		...
		<div class="fill-form-con">
			<div class="clearfix fill-border">
				<label class="fl label long-key">流拍自动上架</label>
				<div class="fr fill-btn fillbtncur" status_code="1" id="is_passIn">
					<div class="btn re">
						<div class="btn-in ab" style="left:1.8rem;"></div>
					</div>
				</div>
		...
		<!-- 开始时间弹框 -->
		<div class="by-time start-time fixed">
			<div class="ab by-time-con">
				<div class="time-today">
					<div class="time-title"><span>默认时间</span></div>
					<ul class="clearfix">
						<li class="fl" @click="shootNow($event)">立即开拍</li>
					</ul>
				</div>
				<div class="time-today">
		...
		<!-- 截止时间弹窗 -->
		<div class="by-time dead-line fixed">
			<div class="ab by-time-con">
				<div class="time-today">
					<div class="time-title"><span>{{ deadlineArr.today }}</span><strong class="chinese-desc">(今天)</strong></div>
					<ul class="clearfix">
						<li class="fl" v-for="num in datatime" @click="choiceDeadline($event)" v-if="num.id>getHours" :data-num="num.id">{{num.time}}</li>
					</ul>
				</div>
				<div class="time-today">
					<div class="time-title"><span>{{ deadlineArr.tomorrow }}</span><strong class="chinese-desc">(明天)</strong></div>
					<ul class="clearfix">
						<li class="fl" v-for="num in datatime" @click="choiceDeadline($event)" :data-num="num.id">{{num.time}}</li>
					</ul>
				</div>
                <div class="time-today">
                    <div class="time-title"><span>{{ deadlineArr.thirdday }}</span><strong class="chinese-desc">(后天)</strong></div>
                    <ul class="clearfix">
                        <li class="fl" v-for="num in datatime" @click="choiceDeadline($event)" :data-num="num.id">{{num.time}}</li>
                    </ul>
                </div>
...
<script>
	...
	var PluploadMore = require("pluploadMore");
	var pluploadVideo = require("pluploadVideo");
    ...
    export default {
		data:function(){
    		...
    		option: {
				...
				certificateImg: '',     		  // 鉴定证书
				imgsJson : '', /*图片路径转为字符串*/
				baseUrl1 : window.config.img_url,  // 图片服务器
				...
				// plupload 初始化对象
				pluploadObj: {},  
				isAllVideo: true,       // 上传拍品是否全部为视频 默认全部为视频
		...
		mounted: function(){
			...
			$(".picture-bottom .next").click(function(){//图层切换
				...
				_this.isAllVideo = true;
				_this.imgs.forEach( (item,index) => {
					if (item.type == 'img') {
						_this.isAllVideo = false;
					}
				})

				if (desc && len) {
					if (_this.isAllVideo) {// 全部为视频
						_this.tipMsg = "请至少上传一张图片!";
						_this.showPopup()
					} else {
		...
		methods: {
			// 立即开拍
			shootNow: function(event) {
				var curtime = new Date().getHours();
				var selectText = $(event.target).html();
				if(selectText) {
					$("#bytime").text(selectText);
      ...
      closeBigImg: function() {
          ...
          this.showBigImgBol = false;
			...
			fillbond: function(num) {//封装点击弹框
				...
				if(num == 0){ //请求父类分类数据
					...
					ajax.getType(0,key,function(res){
						if(res.status == 200 && vue.isPregnant){
							key = 1; // 需要获取全部分类
							// vue.parentType = res.data;
							res.data.forEach((item,index) => {
								if (item.id != window.config.college_id) {
									vue.parentType.push(item);
								};
							})

							vue.isPregnant = false;
							...
							vue.$nextTick(function(){
								$('.child_types').click(function(){  //一级分类点击事件
									...
									$(".fill-two .two-list").css("max-height",vue.maxheight-$(".fill-two .list-title").height());
									//加最大高度内容过多出现滚动条
									ajax.getType(parent_id,key,function(res){
										if(res.status == 200){
			...
			/**
			 * 选择完开始时间之后 处理截止时间
			 * @param  {string} day   所选开始时间日期
			 * @param  {string} hours 所选开始时间的当前小时
			 * @return {return}       无
			 */
			followUpTimeChoice: function(day,hours) {
				// 移除 汉字描述
				$('.chinese-desc').remove();
				...
				var dataArr = [];
				// 格式化时间
				time = time.replace(/(\d{4}).(\d{1,2}).(\d{1,2}).+/mg, '$1-$2-$3');
				...
				for (var i = start; i < len; i++) {
					...
					var mounth = this.toTwo(date.getMonth() + 1);
					var day = this.toTwo(date.getDate());
					dataArr.push(mounth + '月' + day + '日')
				};

				// 获取原对象的键名
				var keys = ['today','tomorrow','thirdday'];

				// 创建新对象
				for(var key in dataArr) {
					this.deadlineArr[keys[key]] = dataArr[key];
				}
			},

			deadlineEvent: function() { // 截止时间
				var vue = this;
				...
				var text = $('.fill-money').html();
				var timeArr = text.split(' ');
				var day = timeArr[0];
				var hours = timeArr[1];
				if (text != '立即开拍' && text != '选择时间') {
					this.followUpTimeChoice(day,hours);
				};
			...
			choicetime: function(event){//点击时间点
				var curtime= new Date().getHours();
				var time=$(event.target).attr("data-num");
				// this.getHours = curtime;
				if(time){
					var day = $(event.target).parent().siblings().children("span").text();
					var hours = $(event.target).text();
					// var html=$(event.target).parent().siblings().children("span").text() + ' ' + $(event.target).text();
					var html = day + ' ' + hours;
					$("#bytime").text(html);
					$(event.target).parents(".by-time").hide();
			...
			toUpdateValue: function() {
				...
				this.certificateImg = this.shelfData.certificate;
				// 传递后端图片数据
				this.formatImgs();
      ...
      formatDesc: function (content) {
      	...
      	// 后端处理
			...
			subdata : function(){
				...
				var begin_time = $('#bytime').text(); // 开始时间
				var end_time = $('#shooting-time').text();   // 截止时间
				...
				var is_passIn = $('#is_passIn').attr('status_code');
				var deposit_price = Number( $('#key2').text() );
				var once_price = Number( $('#fill-price').val() || 0 );
				var refer_price = Number( $('#refer_price').val() || 0 );
				var format_desc = this.formatDesc($('.picture-textarea').val())
				var desc = this.emoji2Str( format_desc );
				...
				/** modify end **/
					...
					ajax.postPublish(all_data,function(res){
						if(res.status == '200'){
							...
							if(all_data.deposit_price != 0 || res.data.status == 6){  
								// 用户保证金设置为0 或者 未交纳保证金 跳转支付页面
								/* iphone返回 缓存数据 引起发布按钮不能点击的bug */
								$('#submit-btn').attr("disabled",false);
								/* end */
