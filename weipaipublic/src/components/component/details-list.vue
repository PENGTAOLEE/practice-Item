	...
	<div class="detailslist" v-if="detailsBoll">
			...
			<div class="follow-list">
				<div class="follow-author clearfix">
					...
					<router-link :to="'/shops/'  + item.user_id + '?type=2' " class="clearfix fl">
						...
						<div class="author fl">
							<div class="icon-box clearfix fl">
                <span class="spritesmap user-level fl" v-if="item.rank" :class="'s'+item.rank"></span>
              </div>
							<div class="name fl" v-if="item.name !== undefined">{{ item.name }}</div>
							<div class="name fl" v-if="item.nick !== undefined">{{ item.nick }}</div>
							<!-- weipai@1.1.1 -->
							<div class="icon-auth-list clearfix">
								<span>
									<span class="dibvam spritesmap score_icon"></span>
									<span class="icon-list-text">{{ parseFloat(item.mark).toFixed(2) }}分</span>
								</span>
								<span v-if="item.student">
									<span class="dibvam spritesmap" :class="{'student_ident':item.student==1,'':item.student==0}"></span>
									<span class="icon-list-text">{{ item.category_name }}</span>
								</span>
              	<span>
              		<span class="dibvam spritesmap auth-user" v-if="item.is_apply" :class="{'personal':item.is_apply==1,'shop':item.is_apply==2}"></span>
              		<span class="icon-list-text" v-if="item.is_apply" :class="{'personal':item.is_apply==1,'shop':item.is_apply==2}"></span>
              	</span>
              </div>
							<!-- weipai@1.1.1 end -->
						</div>
					...
					<!-- 鉴定证书 @weipai1.1 -->
					<div class="spritesmap identification-certificate fr" v-if="item.certificate" :data-src="baseUrl + item.certificate">
						<img :src="baseUrl + item.certificate" alt="" style="opacity:0;">
					</div>
					<!-- 鉴定证书 @weipai1.1 end -->
				</div>
				<p class="follow-copy re text-overflows">
					<router-link :to="'/lotdetails/'+item.id" v-if="!isDetailsDesc" style="color:#333;font-size: 1.4rem" class="text-overflows" v-html="item.desc"></router-link>
					<span v-if="isDetailsDesc" style="color:#333;font-size: 1.4rem" class="text-overflows" v-html="item.desc"></span>
					
					<span class="ab copybtn" v-if="item.desc.length >= 75 || /<br/.test(item.desc)" @click="copybtn($event)" data-statistics="click_descall">显示全部</span>
        </p>

				<ul class="list-img clearfix">
					<!-- 微信预览 -->
					<li class="fl" v-for="(itemImg,indexImg) in item.images" v-if="isWeixin">
						<img v-if="itemImg.type == 'img' " class="list-img-ele" data-type="img" :src="baseUrl + itemImg.url + '?x-oss-process=style/erweima_mark'" alt="goods img" data-statistics="click_img"> 
						<div v-if="itemImg.type == 'video' " class="video-box">
							<img class="video-box-img" :src="videoPoster" alt="">
							<video
					...
					<!-- 浏览器预览 -->
        	<li class="fl" v-for="(itemImg, indexImg) in webImgList" v-if="!isWeixin">
						<img class="preview-img list-img-ele fl" v-if="itemImg.type == 'img'"  :src="itemImg.src" height="100" @click="$preview.open(indexImg, webImgList, option)" data-statistics="click_img">
					</li>
					<li class="fl" v-for="(itemImg, indexImg) in webVideoList" v-if="!isWeixin">
						<div v-if="itemImg.type == 'video' " class="video-box">
							<video :src="baseUrlVideo + itemImg.videoSrc" preload="load" :poster="videoPoster"></video>
							<span class="video-bg" @touchend="showBigImgBrowser($event)" data-type="video"></span>
						</div>
					</li>
				</ul>
				<!-- 点赞人列表 -->
				<ul class="list-people clearfix re peoplemore" v-if="item.likeImg.length != 0">
					<li class="fl divbg people" v-for="itemLike in item.likeImg">
				...
				<div class="follow-state clearfix">
					<div class="fl dibvam free-shipping" v-if="item.is_mail">包邮</div>
					<div class="fl dibvam return" v-if="item.is_refund">包退</div>
					<div class="fl dibvam off-shelf" v-if="item.is_self && (item.status==1||item.status==8)" @click="showPopup">下架</div>
					<div class="fr time dibvam">
						<span class="fl shotsprites"></span>
						{{ item.created_at }}
					</div>
				</div>
				<!-- 正在拍卖 -->
				<div v-if="(item.status == 1 || item.status == 2) && item.nowTime >= item.begin_time"
					class="cutdown-box on-line clearfix" :data-status="item.status" :id="'time_down_'+ item.id">
					<div
						class="follow-time keynum dibvam"
						:data-startTime="item.inputtime"
						:data-endTime="item.end_time"
						:data-id="index"
					>距离结束：
						<div class="cutdown-timebox"><span>00</span>时<span>00</span>分<span>00</span>秒</div>
					</div>
					<div class="img-interactive dibvam follow fr">
						<span class="dibvam imgbrowse">{{ item.views }}</span>
						<span
							class="dibvam imglike"
						...
						>{{ item.likes }}</span>
						<!-- 分享模块 -->
						<!-- <button class="dibvam share-button" :class="'share-button-'+item.id" @click="shareEvents(item.id,index)" :data-goodsid="item.id" v-if="isDetails">
							<i class="dibvam spritesmap share_icon"></i>
							<span class="dibvam share_icon_span">分享</span>
						</button> -->
					</div>
					<!-- :class="{'online-self':item.is_self}" -->
					<div
						data-statistics="click_bid"
						class="follow-offer startoffer online"
						:data-mobile="item.nowmobile"
						:data-bidnum="item.bidtype"
				...
				<!-- 预展中拍品 加一个字段判断是否输入预展中拍品 -->
				<div v-if="item.status == 8 && item.nowTime <= item.begin_time" class="cutdown-box clearfix" :data-status="item.status" :id="'time_down_'+ item.id">
					<div
						class="follow-time keynum dibvam"
						:data-startTime="item.inputtime"
						:data-endTime="item.end_time"
					...
					<div class="follow-offer startoffer">预展中</div>
				</div>

				<!-- 截拍拍品 仍然显示浏览量 Heeeee -->
				<div 
					class="cutdown-box clearfix grey" 
					:class="{'end-offer-grey': item.status !=0 && item.status !=3 && item.status !=4 && item.status !=5 && item.status !=6 && item.status !=7}" 
				...
				>
					<div class="img-interactive dibvam follow fr">
						...
						<span
							...
							@click="doLikes"
						>{{ item.likes }}</span>
						<!-- <button class="dibvam share-button" @click="shareEvents(item.id,index)" :class="'share-button-'+item.id" :data-goodsid="item.id" v-if="isDetails">
							<i class="dibvam spritesmap share_icon"></i>
							<span class="dibvam share_icon_span">分享</span>
						</button> -->
					</div>
				</div>
				<div v-if="item.status == 0 || item.status == 3 || item.status == 4 || item.status == 5 || item.status == 6|| item.status == 7" class="follow-offer grey"> 拍卖结束</div>
						...
						<div class="list-item" v-if="item.bid.data">
							<li v-for="(bidItem,bidIndex) in item.bid.data" :data-userid="bidItem.user_id">
								<div class="clearfix offer-in">
									<router-link :to="'/shops/' + bidItem.user_id + '?type=0'">
									...
									<div class="fr state">
										¥{{ bidItem.bid_price }}
										<span v-if="bidIndex == 0 && ((item.nowTime > item.end_time) || (item.status == 0 || item.status == 3 || item.status == 4 || item.status == 5 || item.status == 6|| item.status == 7))" class="deal-one deal-one-complete">成交</span>
										<span v-if="bidIndex == 0 && (item.status == 1 || item.status == 2) && (item.nowTime < item.end_time)" class="deal-one">领先</span>
			...
			<div class="offer-box fixed" @click="cancelOfferOuter($event)">
				...
				<div class="fixed offerbg offer-money" v-show="showMoney">
					...
					<div class="offer-btn-box" v-if="item.once_price != 0">
						<div class="price-fixed" @click.stop="setOncePrice" :data-onceprice="item.once_price" data-statistics="click_onceprice">一口价 ￥{{ item.once_price }}</div>
						<div class="offerbtnbg offer-lpt" :data-goodsid="item.id" :data-onceprice="item.once_price" @click.stop="oncePrice" data-statistics="click_bidsuccess">出价</div>
					</div>
					<!-- 拍品未设置一口价 -->
		...
		<div class="poupu-box" v-show="showBigImgBol">
			...
			<div class="popup-video-box">
				<video id="my-video" :src="videoSrc" autoplay >
					您的浏览器不支持浏览该视频，请升级版本
				</video>
			</div>
		</div>
		...
		<div class="qrcode-wrap" @click="colseQrcodeWrap" v-if="qrcodeWrap">
			<div class="qrcode-content">
				<!-- 应急解决方案 -->
				<img src="../../assets/img/qrcode_weixin.png" alt="">
				<!-- <div class="qrcode">
					<img src="../../assets/img/qrcode_xiaofupai.png" alt="">
				</div>
...
export default {
    data: function() {
        return {
            ...
            videoPoster: require('../../assets/img/video_ns_2.jpg'),
    ...
    created: function() {
    	this.isDetails = this.VerificatUrl() || true;
    	this.isDetailsDesc = this.VerificatUrl();
    	this.storeObj = this.details[0];

    	var nowId = this.details[0].nowuid;
            ...
            try {
                if (from == 'complete') {
                    ...
                    $('.goods-id'+goodsid).find('.offer-box').show();
                     ...
                     // 获取需要的参数
                    this.bidnum         = $offerNode.attr("data-bidnum");
    ...
    watch: {
        '$route' : ["formatImgs","winReload","wxPreview"]
    },
	methods: {
		// 微信预览图片
		wxPreview: function() {
	    ...
	    setTimeout(function() {
      	// 拍品预览
        utils.wxPreviewImage($('.follow-list .list-img-ele'),'.list-img');

        // 鉴定证书预览
				utils.wxPreviewImage($('.identification-certificate img'),'.identification-certificate');
      },1000)
		},
		...
		// 分享弹窗
		shareEvents: function(id,index) {
			this.storeObj = this.details[index];
			this.showPopupShare = true;
			$('.share-button-'+id).attr('disabled',true)
		},
		...
		// 验证当前页面是否是详情页
		VerificatUrl: function() {
			var str = this.$route.path;
			var reg = /lotdetails/ig;
			return reg.test(str)
		},
    ...
    doLikes: function(ev) {
        ...
        if (!liked) {
            ajax.userLike(id,function(data) {
                if (data.status == 200 || data.status == 502) {
          ...
          if (typeof item.images == 'string') {
            ...
            item.images && (item.images = JSON.parse(  item.images  ) )
        		for (var i = 0; i < item.images.length; i++) {
        			if (item.images[i].type == 'img') {
        				this.webImgList.push({
	              	src: this.baseUrl + item.images[i].url,
				          w: 1200,
				          h: 900,
				          pid: 'img-'+i,
				          videoSrc: item.images[i].url,
				          type: item.images[i].type
	              })	
        			} else {
        				this.webVideoList.push({
	              	src: this.baseUrl + item.images[i].url,
				          w: 1200,
				          h: 900,
				          pid: 'img-'+i,
    ...
    showBigImgBrowser(ev) {
    	var video = $(ev.target).parent('.video-box').find('video');
    	utils.fullScreen(ev,video)
    },
    ...
    // 文案控制
      ...
      if($node.parent().hasClass("text-overflows")){
          ...
          $node.siblings('span').addClass('text-overflows')
          $node.html("显示全部");
      }else{
          $node.parent().find('a').removeClass('text-overflows');
          $node.parent().addClass("copy-stop");
          $node.siblings('span').removeClass('text-overflows')
          $node.html("收起");
      }
    },
    ...
    validateBid: function(ev) {
        ...
        this.bidnum         = $(ev.target).attr("data-bidnum");
        ...
        this.islogin        = $(ev.target).attr("data-islogin");
        ...
        if ( myId == userid) {// 不能给你自出价
            ...
            $('.goods-id'+id).find('.offer-box').hide();
        } else if ( !this.myMobile && (this.islogin==1)) {
            // 验证手机号码
    ...
    startoffer: function(){
        //如果当前价等于起拍价，则为无人出价
        if(this.maxbid == 0 || this.bidnum == 0){
            //如果是第一次出价
            //没有起拍价
    ...
    **/
    ...
    oncepriceOk: function(goodsId) {
        ...
        $('.goods-id'+goodsId).find('.deal-one').addClass('deal-one-complete');
        $('.goods-id'+goodsId).find('.cutdown-box .follow-time').hide();
        $('.goods-id'+goodsId).find('.startoffer').hide()
        $('.goods-id'+goodsId).find('.end-offer-grey').show();
        $('#time_down_'+goodsId).hide()
        this.$nextTick(function(){
...
.deal-one-complete {
	...
	height: 2.5rem;
	line-height: 2.2rem;
	background: url('../../assets/img/chengjiao_bg.png') no-repeat!important;
...
.online-self {
	background-color: #1ab7ef!important;
}
.online-self:after {
	content: "分享给朋友们"
}
...
.follow-list .follow-author .icon-auth-list {
	display: block;
	margin-left: -1rem;
...
.follow-list .follow-state .time span{
	width: 1.7rem;
	height: 1.7rem;
	margin-right: .5rem;
}
...
.identification-certificate {
	...
	margin-top: 1.5rem;
	margin-right: 1.8rem;
	background-position: -8.6rem -25.15rem;
	/* 注释 weipai@1.1.0 */
	/*background-position: -25.45rem -0.85rem;*/
}
...
.follow-list .copy-stop .copybtn{
	...
	padding-left: 1.6rem;
	width: 9rem;
	background: url(../../assets/img/stop.png) no-repeat left center;
...
.share_icon {
	...
	height: 1.25rem;
	font-size: 1.2rem;
	background-position: -14.7rem -27.25rem;
}
