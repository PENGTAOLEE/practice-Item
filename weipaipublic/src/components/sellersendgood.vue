...
<script>
    ...
    var shareSub = require("@/components/component/shareSub");
    var Express = require("@/components/component/express");
        ...
        methods: {
            getInfo: function(){
                ...
                ajax.getSendInfo(order_id,function(data){
                    if(data.status==200){
                        ...
                        that.expressNamePar = data.data.oldexpress||'中通速递';
            ...
            commonSend: function(order_id,express,express_number,is_deliver_home,seller,seller_mobile,seller_address){
                ...
                ajax.sendgood(order_id,express,express_number,is_deliver_home,seller,seller_mobile,seller_address,function(data){
                        if(data.status==200){
                            console.log("发货成功");
                            that.$router.replace("/sellerorderdetails/"+order_id+'?from=sellersendgood');
                        }else{
                            console.log(data.msg);
                        }
                    });
...
.analog-radio {
	display: inline-block;
	padding-right: 3rem;
	background: url(../assets/img/radio_off.png) no-repeat right center;
	background-size: 2rem auto;
}
.on-select {
	background: url(../assets/img/radio_on.png) no-repeat right center;
}
...
.icon-scan {
	background: url(../assets/img/scan.png) no-repeat center center;
