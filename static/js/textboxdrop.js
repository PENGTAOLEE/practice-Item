...
var TextBoxDropCtl = {
	Init: function(config){
		var t = new TextBoxDrop(config.Box,config.MsgBox);
		...
		if(config.ClipTop == undefined){
			//config.ClipTop = 23;
			config.ClipTop = $("#" + config.Box).height();
		}
		
		
		t.DefaultKey = config.DefaultKey;
		t.SetAjaxMethod($.get);
		t.Url(config.Url);
		...
		t.SetContentStyle(function(intput,contentbox){
			...
			if($.browser.ie){
				...
				contentbox.style.top = inputBox.offset().top + config.ClipTop;
			}
			else{
				$(contentbox).css({
					top : inputBox.offset().top + config.ClipTop,
