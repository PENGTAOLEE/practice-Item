...
<div>
<div class="hint">
        <div class="hint_item bdb" v-show="(data.is_refund==2&&data.status!=8)||data.status==17">
            <div class="hint_title">已确认收货，交易完成 </div>
            <span class="hint_des">收货时间：{{data.updatetime}}</span>
        </div>
        ...
        <div class="hint_item bdb" v-show="data.status==7">
        ...
        <div class="hint_item bdb" v-show="data.status==11">
            ...
            <span class="hint_des">付款时间：{{data.updatetime}}</span>
        </div>
    ...
    <div class="hint">
        <div class="hint_item bdb" v-show="data.status==12||data.status==4||data.status==9">
            <div class="hint_title">交易失败</div>
            <span class="hint_des" v-show="data.status==16">失败原因： 买家48小时后申请退货</span>
            <span class="hint_des" v-show="data.status==12">失败原因： 卖家未发货</span>
            <span class="hint_des" v-show="data.status==9">失败原因： 买家拒绝收货</span>
            <span class="hint_des" v-show="data.status==4">失败原因： 买家未按时付款</span><br>
            <span class="hint_des">结束时间：{{data.updatetime}}</span>
        </div>
        <div class="hint_item bdb" v-show="(data.status>=3&&(data.status<=8))||(data.status>=13)">
            <div class="hint_title">已发货</div>
            <span class="hint_des">发货时间：{{time.sendTime}}</span>
        </div>
        <div class="hint_item bdb" v-show="data.is_ftf_pay==4||data.is_ftf_pay==5">
            <div class="hint_title">已发货</div>
            <span class="hint_des">发货时间：{{time.ftfSendTime}}</span>
        </div>      
        <div class="hint_item bdb" v-show="data.status!=4&&data.status!=0&&data.is_ftf_pay!=1&&data.is_ftf_pay!=4&&data.is_ftf_pay!=5&&data.status!=1&&data.status!=10">
            ...
            <router-link :to="'/lotdetails/'+data.goods_id">
                <div class="list-item_bd clr"  >
                    ...
                    <div class="fl list-item_area">
                        ...
                        <div class="list-item_price-status" v-show="(!(data.status==8&&data.is_refund==2))&&(data.status!=12)&&(data.status!=4)&&data.status!=9&&data.status!=16">成交金额：￥{{data.price}}</div>
            ...
            time: function(){
                ...
                for(var i=0;i<list.length;i++){
                    ...
                    switch (type)
                    {
                        ...
                        //    13超时未发货买家申请退款成功
                        case 13:
                            timeDetails["timeoutapppsuc"] =  list[i].log_inputtime;
                            break;
