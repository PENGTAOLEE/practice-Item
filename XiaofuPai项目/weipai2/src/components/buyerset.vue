...
<div class="buyerSet">
	...
	<div class="buyerList">
		<form action="#">
			...
			<div class="buyerList-group cc" v-show="data.address_id==0" >
				<label>收货地址</label>
				<router-link to="/location?from=buyerset"  title="" class="buyserA">设置收货地址</router-link>
				<em class="compete-row"></em>
			</div>
			
			<div class="compete-site compete-address compete-address-1 cc" v-show="data.address_id!=0"> 
				<span class="compete-site-sp">收货地址</span>
				<router-link to="/location?from=buyerset"  title="" class="compete-p">{{data.address_name+data.address_mobile+data.address_area+data.address}}</router-link>
				<em class="compete-row"></em>
			</div>
	...
	<my-footer></my-footer>
	<!--  手机号-->
	<div class="offer-box fixed" v-show="mobileBox">
	...
	<div class="offer-box fixed" v-show="identifyBox">
			...
			refreshUserData: function(){
				...
				ajax.refreshUserData(contact_name,is_seller,function(data){
					if(data.status == 200){
						// that.$router.go(0);
						// that.data = data.data;
						console.log("个人中心页信息编辑成功！");
						console.log("联系人为："+contact_name);
					
						console.log("固定默认设置是否为卖家为："+is_seller);
