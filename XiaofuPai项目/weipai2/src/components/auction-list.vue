...
<div class="swiper-container-auction">
            ...
            <div class="content-slide content-slide-border"  v-for="(item,index) in dataList">
            	<!-- 跳转拍品详情 router-link to="/lotdetails/1"-->
					...
					<router-link :to="/lotdetails/ + item.id">
		              	<dl class="content-dl cc">
			              	...
			              	<dd>
			              		...
			              		<!-- 竞拍中 -->
			              		<span v-if="item.auction_type == 1">当前价格：￥{{ item.current_price }}</span>
			              		<span v-if="item.auction_type == 1">出价次数：{{ item.bid_num }}次</span>
			              		<!-- 已节拍 -->
              	...
              	<div class="btn-box clearfix">
					...
					<!-- 草稿箱按钮 -->
			        <strong v-if="item.auction_type == 6" class="content-Btn content-Btn-sj" :data-id="item.id" :data-status="item.status" @click="shelves">上架</strong>
		            <strong v-if="item.auction_type == 6" class="content-Btn content-Btn-abolish" :data-id="item.id" :data-index="index" @click="confirmDel">删除</strong>
              	</div>
	...
	<!-- loading -->
	<div class="loading">
		<img src="/res/img/loading.gif" alt="" v-if="loading">
